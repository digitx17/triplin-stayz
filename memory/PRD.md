...
- Section "Things I've built" (Triplin/Travel CRM/Hospitality CRM + CaseStudyModal) REMOVED from page per user (2026-09-18); files kept but unused (ThingsIveBuilt.tsx, CaseStudyModal.tsx, PROJECTS data). Nav "Work", hero CTA and contact "View my work" now target #gallery. Section numbers: 04 Marketing Work, 05 Travel, 06 Approach, 07 Skills, 08 Why travel, 09 Contact.
- 2026-09-18 restructure: removed "On the road" and "My Approach" sections. Page = Hero / How It Started / Experience / Marketing Work (04) / Skills (05) / Toolkit marquee strip / Why travel (06) / Contact (07). Marketing filters: All, Photoshoot, Website & CRM, Graphic Design / Content, Events, Influencer Collab, Listings; dashboard uploads (any section) appear in gallery under "Graphic Design / Content". Toolkit = marquee strip (ToolkitStrip.tsx). CMS-lite: GET/PUT /api/content (routers/content.py, Mongo site_content) holds skills clusters + toolkit items; edited via ContentEditor in /admin; frontend hook lib/content.ts useSiteContent() with data.ts defaults as fallback.
